...
// ── HTML 디코딩 함수 ────────────────────────────────
// 이중 이스케이프된 문자열이나 따옴표를 실제 HTML 태그로 변환합니다.
const decodeHtml = (html: string) => {
  if (!html) return "";
  let cleanHtml = html.replace(/(^"|"$)/g, "").replace(/\\"/g, '"');
  
  // 서버 렌더링 시에는 임시로 문자열 반환
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        ...
        {/* 설명 내용: dangerouslySetInnerHTML 적용 */}
        <div className="relative">
          <div
            className={`prose prose-sm dark:prose-invert max-w-none text-gray-700 dark:text-gray-300 leading-relaxed [&_img]:max-w-full [&_img]:h-auto [&_img]:block [&_img]:mx-auto ${
          ...
          {(product.description || product.descriptionKo) && (
            <>
              ...
              <button
                ...
                className="relative z-10 w-full mt-4 py-4 text-base font-semibold bg-white dark:bg-gray-950 border border-gray-200 dark:border-gray-800 rounded-xl text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 flex items-center justify-center gap-2 transition-all shadow-sm"
              >
                {descExpanded ? (
                  <>상품 설명 접기 <ChevronUp size={18} /></>
                ) : (
                  <>상품 설명 더보기 <ChevronDown size={18} /></>
                )}
              </button>
            </>
          )}
